...
public struct Enhanced: Condition, ConditionSnapshot {
    ...
    public init(stat: AttributeName, amount: Int, in gameSession: isolated GameSession = #isolation)
    {
        self.id = gameSession.nextId()
        self.stat = stat
        self.amount = amount
...
public struct Exhausted: Condition, ConditionSnapshot {
    public let id: Int
    public var amount: Int
    public init(amount: Int, in gameSession: isolated GameSession = #isolation) {
        self.id = gameSession.nextId()
        self.amount = amount
    }
}
